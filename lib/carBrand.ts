// 차량명(제조사가 안 적혀있어도 모델명만으로) 브랜드를 추정하는 로직.
// cavior 웹(lib/carBrand.ts) · ChavatarApp(utils/carBrand.ts)과 동일한 규칙 — 세 곳 모두
// 같은 정적 로고(carvior.store/brand-logos)를 참조하므로 규칙을 어긋나지 않게 유지할 것.
...
export function brandLogoUrl(text: string | null | undefined): string | null {
  const slug = detectBrand(text)?.slug;
  return slug ? `https://carvior.store/brand-logos/${slug}.png` : null;
}

export function brandLabel(text: string | null | undefined): string | null {
  return detectBrand(text)?.label ?? null;
}
